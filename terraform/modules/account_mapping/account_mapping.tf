    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${var.eks_oidc_provider.url}:sub" = "system:serviceaccount:${var.kubernetes_namespace}:${var.kubernetes_service_account_name}"
          }
        }
      }
    ]
...
locals {
  service_account_gcp_role_member = "serviceAccount:${var.gcp_project}.svc.id.goog[${var.kubernetes_namespace}/${kubernetes_service_account.account.metadata[0].name}]"
}
...
# Allows the Kubernetes service account to request access and identity tokens
