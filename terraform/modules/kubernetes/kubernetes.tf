...
# features for automaticaly providing a service account token (e.g. via a
...
# account in GCP-level policies, below.
locals {
      ...
      spec {
        template {
          ...
          spec {
            container {
              name  = "facilitator"
              image = "us.gcr.io/prio-bringup-290620/test-facilitator:0.0.12" #"${var.container_registry}/${var.execution_manager_image}:${var.execution_manager_version}"
