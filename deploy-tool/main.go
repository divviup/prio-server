...
func backupKeys(
	...
	if gcpProjectName == "" {
		log.Printf("no GCP project name in Terraform output -- skipping key backup")
		return nil
	}
	...
	for keyName, _ := range manifestWrapper.SpecificManifest.BatchSigningPublicKeys {
		keyNames = append(keyNames, keyName)
	}

	for keyName, _ := range manifestWrapper.SpecificManifest.PacketEncryptionKeyCSRs {
		keyNames = append(keyNames, keyName)
	}
	...
	for _, keyName := range keyNames {
		// Secrets backup storage is not namespaced like Kubernetes, so we
		// concatenate the namespace and key name to get a unique name
		uniqueKeyName := fmt.Sprintf("%s-%s", manifestWrapper.KubernetesNamespace, keyName)
		exists, err := secretsBackup.ContainsSecret(uniqueKeyName)
		if err != nil {
			return fmt.Errorf("unable to determine if secret %s exists: %w", uniqueKeyName, err)
		}

		if exists {
			log.Printf("secret %s already backed up", uniqueKeyName)
			continue
		}

		log.Printf("backing up secret %s", uniqueKeyName)
		...
		if err := secretsBackup.BackupSecret(uniqueKeyName, secret.Data["secret_key"]); err != nil {
			return fmt.Errorf("failed to backup secret %s: %w", uniqueKeyName, err)
		}
	}
...
func main() {
	...
	if *kubeConfigPath == "" {
		...
		if err != nil {
			log.Fatalf("failed to lookup current user: %w", err)
		}
	...
	for dataShareProcessorName, manifestWrapper := range terraformOutput.SpecificManifests.Value {
		...
		if err := backupKeys(k8sClient, terraformOutput.GcpProject.Value, &manifestWrapper); err != nil {
			log.Fatalf("%s", err)
		}
	}
}
