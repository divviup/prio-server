use crate::{
    ...
    logging::{EVENT_KEY_IDENTITY, EVENT_KEY_TASK_ACKNOWLEDGEMENT_ID, EVENT_KEY_TASK_QUEUE_ID},
...
impl<T: Task> GcpPubSubTaskQueue<T> {
    pub fn new(
        ...
        let logger = parent_logger.new(o!(
            ...
            EVENT_KEY_TASK_QUEUE_ID => subscription_id.to_owned(),
...
impl<T: Task> TaskQueue<T> for GcpPubSubTaskQueue<T> {
    ...
    fn acknowledge_task(&mut self, handle: TaskHandle<T>) -> Result<()> {
        info!(
            ...
            EVENT_KEY_TASK_ACKNOWLEDGEMENT_ID => &handle.acknowledgment_id,
        );
